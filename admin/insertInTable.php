...
if (isset($_POST['generate-submit'])) {
    ...
    if (!empty($_FILES["file"]["name"])) {
        ...
        $allowTypes = array('pdf', 'doc', 'docx','mb');
        if (in_array($fileType, $allowTypes)) {
            // Upload file to server
            if (move_uploaded_file($_FILES["file"]["tmp_name"], $targetFilePath)) {
                // Insert file name and file path into database
                $sql = "INSERT INTO tender(TenderNo,Department,Description,Price,DueDate,Duration,filename,filetype) VALUES('$tenderNo','$dept','$desc','$price','$dueDate','$duration','$fileName','$targetFilePath')";
